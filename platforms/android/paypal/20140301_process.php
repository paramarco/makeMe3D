...
{    
	...
	$number_figures = $_POST["number_figures"];
	...
	switch($size) {
		...
		$ItemPrice = $price; 
		$ShippingPrice = 30; 
		$ItemTotalPrice = $price + ($price * 0.21) + $ShippingPrice;
		$ItemTax = $price * 0.21;
		
	//Check if there is any artist available today for this request
	switch($material) {
		            case 1 :  // hyper realistic
		            	$sql_query = "SELECT id, email, name
	...
	$transactions = mysql_query($sql_query);
	
	if (mysql_affected_rows($link)==0)	{	$noArtistAvailable = TRUE;	}
    ...
    $padata =   '&CURRENCYCODE='.urlencode($PayPalCurrencyCode).
                ...
                //'&PAYMENTREQUEST_1_ITEMAMT='.urlencode($ItemTotalPrice).
                //'&PAYMENTREQUEST_0_SHIPPINGAMT='.urlencode($ShippingPrice).
               // '&PAYMENTREQUEST_0_TAXAMT='.urlencode($ItemTax).
                "&SOLUTIONTYPE=Sole".
                //'&L_PAYMENTREQUEST_0_QTY0=1'.
                '&L_PAYMENTREQUEST_0_AMT0='.urlencode($ItemTotalPrice).//'&L_PAYMENTREQUEST_0_AMT0='.urlencode($ItemPrice).
                '&L_PAYMENTREQUEST_0_NAME0='.urlencode($ItemName).
                //'&L_PAYMENTREQUEST_1_AMT0='.urlencode($ItemTotalPrice).//'&L_PAYMENTREQUEST_0_AMT0='.urlencode($ItemPrice).
                //'&L_PAYMENTREQUEST_1_NAME0='.urlencode($ItemName).
...
if(isset($_GET["token"]) )
{
	...
	$padata =	'&TOKEN='.urlencode($token).
      ...
      if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"]))
      {
			...
			//$token4Update = urlencode($httpParsedResponseAr['TOKEN']);
			...
			switch($material_bought) {
		            case 1 :  // hyper realistic
		            	$sql_query = "SELECT id, email, name, fotoPath, website
										FROM availability A, tz_members M
										WHERE M.id = A.idMember
										AND A.lastDay > NOW() 
										AND M.workWith3D = 1";
		...
		if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
          {
			...
			$email2customer .= "<tr><td><strong>Artist's email:</strong> </td><td>" . $row["email"]. "</td></tr>";	
